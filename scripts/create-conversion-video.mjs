...
function run(command, args, options = {}) {
  const result = spawnSync(command, args, { stdio: 'inherit', timeout: Number(process.env.FFMPEG_TIMEOUT_MS || 180000), ...options });
...
function productFromConfig(productId) {
  ...
  return topProducts.find((p) => p.id === productId) || { id: productId, name: productId, funnelUrl: `/product/${productId}`, checkoutUrl: `/checkout?productId=${productId}&coupon=SAVE15` };
}
function listProductImages(productId) {
  ...
  return files.sort((a, b) => {
    const score = (file) => { const lower = path.basename(file).toLowerCase(); if (lower.includes('main')) return 0; if (lower.includes('front')) return 1; if (lower.includes('bottle')) return 2; return 5; };
...
function buildPlan(product) {
  return {
    ...
    scenes: [
      ...
      { duration: 3, text: 'Save 15 percent on your first order.\nnatureswaysoil.com/lawn-repair', type: 'product' }
    ]
  };
}
async function fetchPexelsVideo(query, outFile) {
  ...
  try {
    ...
    for (const video of data.videos || []) {
      const files = (video.video_files || []).filter((f) => f.link && f.width && f.height).sort((a, b) => (b.height || 0) - (a.height || 0));
...
function writeTextFile(text) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'nws_text_'));
...
function normalizeBrollScene(inputFile, text, outFile, duration) {
  const textFile = writeTextFile(text);
  const filter = `scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,trim=duration=${duration},setpts=PTS-STARTPTS,drawtext=textfile=${textFile}:fontcolor=white:fontsize=62:box=1:boxcolor=black@0.42:boxborderw=22:x=(w-text_w)/2:y=1450:line_spacing=14`;
  run('ffmpeg', ['-y', '-hide_banner', '-loglevel', 'error', '-i', inputFile, '-t', String(duration), '-vf', filter, '-an', '-c:v', 'libx264', '-preset', 'ultrafast', '-pix_fmt', 'yuv420p', '-r', '30', outFile]);
}
...
function concatenateScenes(sceneFiles, outFile) {
  ...
  run('ffmpeg', ['-y', '-hide_banner', '-loglevel', 'error', '-f', 'concat', '-safe', '0', '-i', listFile, '-c:v', 'libx264', '-preset', 'ultrafast', '-pix_fmt', 'yuv420p', '-r', '30', '-movflags', '+faststart', outFile]);
}
function exportPoster(videoFile, posterFile) { run('ffmpeg', ['-y', '-hide_banner', '-loglevel', 'error', '-i', videoFile, '-frames:v', '1', '-q:v', '3', posterFile]); }
function qualityGate(videoFile, productImages) {
  ...
  if (size < 100000) throw new Error(`Video too small: ${size}`);
  ...
  const width = Number(stream.width || 0), height = Number(stream.height || 0), duration = Number(stream.duration || 0);
  if (width !== 1080 || height !== 1920) throw new Error(`Wrong size: ${width}x${height}`);
  if (duration < 18 || duration > 31) throw new Error(`Wrong duration: ${duration}`);
  return { width, height, duration, size, productImageCount: productImages.length };
}
async function main() {
  ensureDir(OUT_DIR); ensureDir(GENERATED_DIR);
...
main().catch((error) => { console.error('❌ Conversion video failed:', error.message); process.exit(1); });
