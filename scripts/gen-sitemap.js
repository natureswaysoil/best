...
try {
  ...
  blogSlugs = [...src.matchAll(/"slug":\s*"([^"]+)"/g)].map((match) => match[1]);
} catch (e) {
...
console.log(`gen-sitemap: wrote ${urls.length} urls (${productIds.length} products, ${blogSlugs.length} blog articles) to public/sitemap.xml`);
