...
function firstSize(product: ProductData) {
  return product.sizes?.[0] || { name: '', price: product.price, sku: product.id };
}
...
export default function CartPage() {
  ...
  const systemRecommendations = (matchedRule?.recommendIds || [])
    .map((id) => allProducts.find((p) => p.id === id))
    .filter((p): p is ProductData => Boolean(p && p.inStock && !cartProductIds.has(p.id)))
    .slice(0, 2);

  const freeShippingSuggestion = allProducts
    .flatMap((p) => (p.sizes || [{ name: '', price: p.price, sku: p.id }]).map((s) => ({ product: p, size: s })))
    .filter(({ product, size }) => product.inStock && !items.some((x) => x.productId === product.id && x.sku === size.sku) && size.price >= remaining)
  ...
  const checkout = async () => {
    ...
    try {
      const response = await fetch('/api/create-cart-checkout-session', {
        ...
        body: JSON.stringify({ items, successPath: '/order-success?session_id={CHECKOUT_SESSION_ID}', cancelPath: '/cart' }),
      });
        ...
        {!items.length ? (
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <section className="lg:col-span-2 space-y-5">
              {items.map((x, i) => (
                <article key={`${x.productId}-${x.sku}`} className="bg-white border rounded-xl p-4 flex gap-4 items-center">
                  {x.image && <img src={x.image} alt="" className="w-24 h-24 object-contain" />}
                  <div className="flex-1 min-w-0">
                    <h2 className="font-semibold leading-snug">{x.productName}</h2>
                    <p className="text-sm text-gray-500">{x.sizeName}</p>
                    <p className="font-bold text-green-700 mt-2">${x.price.toFixed(2)} each</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => update(i, x.quantity - 1)} className="border rounded px-3 py-1" aria-label={`Decrease ${x.productName}`}>−</button>
                    <span className="min-w-6 text-center">{x.quantity}</span>
                    <button onClick={() => update(i, x.quantity + 1)} className="border rounded px-3 py-1" aria-label={`Increase ${x.productName}`}>+</button>
                  </div>
                </article>
              ))}

              {matchedRule && systemRecommendations.length > 0 && (
                <section className="rounded-2xl border-2 border-nature-green-200 bg-nature-green-50 p-5">
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    {systemRecommendations.map((product) => {
                      const size = firstSize(product);
                      return (
                        <div key={product.id} className="bg-white rounded-xl border p-4 flex gap-3 items-center">
                          <img src={product.image} alt="" className="w-20 h-20 object-contain" />
                          <div className="min-w-0 flex-1">
                            <p className="font-semibold text-sm leading-snug line-clamp-2">{product.name}</p>
                            <p className="text-xs text-gray-500 mt-1">{size.name}</p>
                            <p className="font-bold text-nature-green-700 mt-1">${size.price.toFixed(2)}</p>
                            <button onClick={() => addProduct(product)} className="mt-2 text-sm font-bold text-nature-green-700 hover:text-nature-green-800">+ Add to my order</button>
                          </div>
                        </div>
                      );
                    })}
            ...
            <aside className="bg-white border rounded-2xl p-6 h-fit space-y-5 lg:sticky lg:top-24">
              ...
              {remaining > 0 ? (
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-green-800 font-semibold">✓ Free shipping unlocked</div>
              )}
